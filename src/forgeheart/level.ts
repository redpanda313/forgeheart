...
export interface Collider {
  ...
  max: THREE.Vector3;
  /**
   * floor — walkable tops (thin); ignore for most horizontal blocking
   * solid — walls / furniture / full obstacles
   * skyway — board-only wind paths between islands (not walkable on foot)
   */
  kind?: 'floor' | 'solid' | 'skyway';
}
...
function floorPlane(
  ...
  mesh.receiveShadow = true;
  mesh.castShadow = true;
  // Thick underside for reliable ground contact; always tagged as floor
  const colH = 0.85;
  const col: Collider = {
