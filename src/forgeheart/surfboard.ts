...
export const BOARD = {
  ...
  camTipAccel: -0.12,
  camTipBrake: 0.14,
  /** Very far from active path before soft correct (sky lanes are wide) */
  offPathLimit: 55,
...
export class Surfboard {
   ...
   * @param freeRoam skip path magnet / bumps / off-path snap (market plaza)
   * @param freeFloorY optional floor top when free-roaming (hover height applied)
   */
  tick(
    dt: number,
    accel: number,
    ...
    const near =
      !freeRoam && path.length >= 2
        ? nearestOnPath(path, pathDist, this.position)
        : {
    ...
    const roadY =
      freeRoam && freeFloorY != null
        ? freeFloorY + BOARD.hoverHeight
        : freeRoam
          ? // No floor under board — fall (kill plane / void rescue elsewhere)
            this.position.y - 80
          : near.point.y + BOARD.hoverHeight;
    ...
    // Soft path magnet / bumps / anti-stuck — race only (market free-roam skips these)
    if (!grinding && !freeRoam) {
      ...
      // Lateral off-path snap only when extremely far — vertical void handled by game kill plane
      latLen = Math.hypot(this.position.x - near.point.x, this.position.z - near.point.z);
      if (latLen > BOARD.offPathLimit) {
